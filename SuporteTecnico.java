...
public class SuporteTecnico {
	...
	public String buscarSolucao(String problema) {
		Interpretadora inter = new Interpretadora(problema);
		for(String palavra : inter.getPalavraChave())
			if (this.bancoSolucao.keySet().contains(palavra))
				return this.bancoSolucao.get(palavra);
		
		
		return this.buscarRespostaPadraoAleatoria();
	}
	
	private String buscarRespostaPadraoAleatoria() {
		Collections.shuffle(this.respostaPadrao);
		
		return this.respostaPadrao.get(0);
	}
